...
`;
  ...
  console.log('  ★ 神技 (一键同步所有环境变量到云端，无需手动在网页复制粘贴):');
  console.log('    终端直接执行:  npx wrangler pages secret bulk .dev.vars --project-name=<你的项目名称>');
  console.log('-------------------------------------------------------------');
  console.log(' 👉 Settings -> Functions -> D1 Database bindings 中添加 (Production 和 Preview 都需添加):');
  console.log(`  Variable name: DB   |   D1 database: ${dbName}`);
  console.log(' ⚠️  极重要提示: 绑定好后，请务必在 Deployments 选择最新记录点击 [Retry deployment]，新绑定才能生效！');
