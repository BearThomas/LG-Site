...
`;
  ...
  console.log('=============================================================');
  console.log('\n【生产与预览环境指引】在 Cloudflare Pages 后台上线时：');
  console.log('  ★ 方法 A：一键同步所有环境变量到云端（无需手动去网页复制粘贴）：');
  console.log('    终端直接执行:  npx wrangler pages secret bulk .dev.vars --project-name=<你的项目名称>');
  console.log('  ★ 方法 B：直接复制粘贴到网页后台 (Settings -> Environment variables 的 Production 和 Preview):');
  console.log('-------------------------------------------------------------');
  console.log(`APPWRITE_ENDPOINT=${appwriteEndpoint}`);
  console.log(`APPWRITE_PROJECT_ID=${appwriteProjectId}`);
  console.log(`APPWRITE_API_KEY=${appwriteApiKey}`);
  console.log(`AUTH_TOKEN_SECRET=${authTokenSecret}`);
  console.log('-------------------------------------------------------------');
  console.log(' 👉 (D1 数据库绑定) 在 Settings -> Functions -> D1 Database bindings 中添加 (Production 和 Preview 都需添加):');
  console.log(`    Variable name: DB   |   D1 database: ${dbName}`);
  console.log(' ⚠️  极重要提示: 配置好后，请务必在 Deployments 页面点击 [Retry deployment] 重新部署一次才能正式生效！');
  ...
  console.log('  1. 一键同步环境变量: npx wrangler pages secret bulk .dev.vars --project-name=<你的项目名称>');
  console.log('  2. 本地联调预览:    npm run dev');
  console.log('  3. 构建并发布上线 (Mac/Linux/CMD):        npm run build && npx wrangler pages deploy dist --project-name=<你的项目名称>');
